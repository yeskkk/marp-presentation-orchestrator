...
def test_migration_v2_retains_every_config(compact_root):
    ...
    c=service.store.connect();c.execute('DROP TABLE releases');c.execute('DROP TABLE decks');c.execute('DROP TABLE gate_runs');c.execute('PRAGMA user_version=2');c.close()
