...
def make_confirmed_task(
    ...
    *,
    ...
    slug, task = create_task(
        ...
        minutes=90 if kind == "course" else None,
    )
...
def initialize_one_deck(
    ...
    *,
    ...
    slug, task = make_confirmed_task(root, kind=kind, stop_mode=stop_mode, slug=slug)
...
def install_fake_marp(root: Path, *, version: str = "9.9.9") -> Path:
...
pages = 1 + sum(1 for line in body if line.strip() == '---')
doc = fitz.open()
