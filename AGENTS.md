...
legacy 模板已经从源码删除，需要时从 Git 历史提取。旧 compact 任务通过 `task open` 自动兼容；不重新初始化。
...
## 返修焦点与现行版本
旧 compact 任务先 `task open <slug>`，只读查询不等于语义返修完成。当前发布 leaf 稿是新返修基线。用户要求整稿表达时使用 `repair open ... --focus student-expression`，新确认范围覆盖旧的“只改练习/不润色”焦点，但不重置教学取舍、模型或未选范围。五通道发现的其他具体问题也必须交作者处置，不能按初始问题族过滤；不新增验修轮次。

### 当前版本与清理
旧任务通过 `task open` 自动兼容，不初始化重建。当前稿由发布关系识别，同 ID 保留项不是父稿替代。当前版本检查点只在确认策略及安全点执行；发生中断先恢复检查点。不要为了旧完整内容的重放重新派发模型。详见 docs/STORAGE-MAINTENANCE.md。
