...
def report(task: Path, presentations=None):
    ...
    def coordinates(aid):
        ...
        return {'presentation':j.get('presentation'),'kind':j.get('kind'),'channel':j.get('channel'),'job_id':j.get('id'),
                'attempt_sequence':a.get('sequence'),'attempt_state':a.get('state'),'model':s.get('model'),'effort':s.get('effort')}
    ...
    for row in usage:
        ...
        selected_attempts.add(row['attempt_id']);ids=attribution[(row['attempt_id'],row['call_id'])]
        ...
        calls.append({**row,**co,'provider_turn_id':provider_turn,'provider_seconds':(b-a)/1000 if interval else None,'provider_interval':interval,'request_id':rid,'operation':q.get('operation'),'attribution':'exact_retained_envelope' if rid else 'unknown_or_ambiguous',
    ...
    for e in events:
        ...
        d=e['detail'];presentation=d.get('presentation')
        if selected and presentation not in selected:continue
        end=ends.get(d['wait_id']);a=stamp(e['created_at']);b=stamp(end['created_at']) if end else None
        if a and b and b>=a:wait_intervals.append((a.timestamp(),b.timestamp()))
        waits.append({'wait_id':d['wait_id'],'reason':d['reason'],'presentation':presentation,'seconds':(b-a).total_seconds() if a and b else None,'open':end is None})
    ...
    result={'version':1,'scope':{'task':str(task.resolve()),'presentations':sorted(selected) if selected else 'all','read_only':True,'model_calls':0},
        'calls_observed':len(calls),'counters':counters(calls),'by_phase':grouped,'calls':calls,
        'coverage':{'attempts_with_usage':len(selected_attempts),'attempts_expected':len(expected),'calls_with_operation':sum(r['operation'] is not None for r in calls),'duration_events':len(durations),'calls_with_provider_timestamps':len(known_turn_seconds),'retained_index':index_coverage,'calls_with_duration_request':sum(any(d['request_id']==r['request_id'] for d in durations) for r in calls if r['request_id'])},
        'timing':{'provider_sum_seconds':sum(known_turn_seconds) if calls and len(known_turn_seconds)==len(calls) else None,'provider_known_sum_seconds':sum(known_turn_seconds),'provider_union_seconds':seconds(model_intervals) if model_intervals else None,
            ...
            'explicit_wait_union_seconds':seconds(clipped_waits),'unattributed_seconds':max(0,window-seconds(observed+clipped_waits)) if window is not None else None,
...
def render(value,format='json'):
    ...
    if format=='csv':
        stream=io.StringIO();fields=['presentation','kind','channel','operation','attempt_id','attempt_sequence','call_id','request_id','model','effort','provider_turn_id','provider_seconds',*FIELDS,'packet_context_bytes','task_context_action','attribution']
    ...
    out=['# Token 与耗时报告','',f"调用记录：{value['calls_observed']}；只读生成，未调用模型。",'', '|课件|阶段|通道|操作|调用数|总 token|供应端秒数|','|---|---|---|---|---:|---:|---:|']
