...
cat >&2 <<'NOTICE'
WARNING: Codex will run without approvals or sandboxing. Use only inside an externally isolated VM/container or dedicated account. See docs/SECURITY.md.
NOTICE
...
exec codex --strict-config --cd "$ROOT" --dangerously-bypass-approvals-and-sandbox --search "$PROMPT"
